 ...
 * rate-data.php as lgp-links.csv, so the proxy reads links from a local file
 * instead of fetching the Google published CSV.
 *
 * Place this file in the SAME folder as rate-data.php (the desk folder).
 * It only accepts CSVs whose header contains operator_code and lgp_links.
 */
...
if (!in_array('operator_code', $header, true) || !in_array('lgp_links', $header, true)) {
    fail('Header must include both "operator_code" and "lgp_links". Found: ' . implode(', ', $header));
}

// Count how many rows carry a non-empty link, for a helpful confirmation.
$ci_code = array_search('operator_code', $header, true);
...
$dest = __DIR__ . '/lgp-links.csv';
if (@file_put_contents($dest, $content) === false) {
    fail('Could not write lgp-links.csv. Check that the desk folder is writable by PHP.', 500);
}

echo json_encode([
    'ok'          => true,
    'saved_as'    => 'lgp-links.csv',
    'rows'        => $totalRows,
