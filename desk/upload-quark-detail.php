<?php
/**
 * upload-quark-detail.php
 * Receives the Quark departure-detail JSON (multipart field "file") and saves it
 * next to rate-data.php as quark-detail.json. rate-data.php then processes it,
...
if (empty($list) || !isset($list[0]['departure_id'])) {
    fail('JSON parsed but does not look like Quark departure details (no departure_id found).');
}
$withCabins = 0;
foreach ($list as $row) {
    if (isset($row['cabins']) && is_array($row['cabins']) && count($row['cabins'])) $withCabins++;
}

$dest = __DIR__ . '/quark-detail.json';
if (@file_put_contents($dest, $content) === false) {
    fail('Could not write quark-detail.json. Check that the desk folder is writable by PHP.', 500);
}

echo json_encode([
    'ok'            => true,
    'saved_as'      => 'quark-detail.json',
    'departures'    => count($list),
    'with_cabins'   => $withCabins,
]);
