<?php
/**
 * upload-swan-links.php
 * Receives an LGP Links CSV (multipart field "file") and saves it next to
 * rate-data.php as swan-lgp-links.csv, so the proxy reads links from a local file
...
$dest = __DIR__ . '/swan-lgp-links.csv';
if (@file_put_contents($dest, $content) === false) {
    fail('Could not write swan-lgp-links.csv. Check that the desk folder is writable by PHP.', 500);
}

echo json_encode([
    'ok'          => true,
    'saved_as'    => 'swan-lgp-links.csv',
    'rows'        => $totalRows,
